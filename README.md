...
- **★（採用マーク）**: 確反技リストの★を押すと「この技への自分の確反」として採用され、
  閉じた状態のヘッダーにも★技名が表示されます（対面ごとのチートシートとして使えます）
- **コンボメモ**: ★を付けた技の下にメモ欄が出ます。確反始動からのコンボを自由記述で保存できます
  （例: `弱昇龍拳 > SA3 ／ 画面端はOD波動〆`）。保存先はブラウザ（localStorage）です
- **★のみ表示**チェックで、採用技を決めた相手技だけに絞った復習モードになります
